...
// ── 前置校验 ──────────────────────────────────────────────────────────────────
if (!process.send) {
  console.error('[component-service] 必须通过 child_process.fork 启动');
  process.exit(1);
}
...
if (typeof core.init !== 'function') {
  const msg = '[component-service] core 必须导出 init() 方法';
  console.error(msg);
...
async function handleMessage(msg) {
  ...
  try {
    ...
    console.error(`[component-service] ${method} 失败:`, err.message);
...
Promise.resolve(core.init()).then(() => {
  ...
  console.log('[component-service] 就绪');
}).catch(err => {
  console.error('[component-service] 初始化失败:', err.message);
